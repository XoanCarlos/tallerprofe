...
class taller():
    def __init__(self):
        ...
        self.lblerrorusu = b.get_object("lblerroraltausu")
        ...
        "on_btnusualta_clicked": self.altausu, }
    ...
    def control(self, widget):
        ...
        self.passc = self.entpass.get_text()
        if moduloscli.correo(self.correo):
            d_pass = conexion.usuario(self.correo)
            if d_pass == self.passc:
    ...
    def altausu(self, widget):
        ...
        if password == password2 and ( password != "" or password2 != "" ):
            self.lblerrorusu.set_text("")
            if conexion.altausuario(mail, password):
                self.lblerrorusu.set_text("Usuario dado de Alta")
        else:
            self.lblerrorusu.set_text("No coinciden los passwords")
